  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file && profile) {
      ...
      reader.onload = (evt) => {
        ...
        const newProfile = { ...profile, avatarBase: result };
      ...
      <div className="px-4 pt-4 pb-2 z-20">
        <header className="bg-white/90 backdrop-blur-sm rounded-full shadow-sm border-2 border-slate-100 p-2 flex items-center justify-between">
            ...
            {isHome ? (
              ...
              >
                <div className="w-12 h-12 rounded-full bg-sky-100 border-2 border-white shadow-sm overflow-hidden group-active:scale-95 transition-transform">
                   <img src={profile?.avatarBase || DEFAULT_AVATAR} alt="Profile" className="w-full h-full object-cover" />
                </div>
                <input 
                  type="file" 
                  ref={fileInputRef} 
            ...
            <div className="flex flex-col">
               {isHome ? (
                 <>
                   ...
                   <span className="text-lg font-black text-slate-800 leading-tight">Mundo d{profile?.gender === 'girl' ? 'a' : 'o'} {profile?.name || 'Miguel'}</span>
                 </>
               ) : (
                 <>
          ...
          <div className="flex items-center">
             {isHome && (
                <button onClick={() => navigate('/profile-setup')} className="w-10 h-10 rounded-full bg-slate-50 border-2 border-slate-100 flex items-center justify-center text-slate-300">
                   <Settings size={18} />
                </button>
             )}
          </div>
